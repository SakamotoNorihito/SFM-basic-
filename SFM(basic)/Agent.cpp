...
Vector2d Agent::drivingForce_e(Room room, const std::vector<Agent>& guide, const std::vector<Agent>& evacuee)
{
	...
	const double sigma_th = 1;												//速度の方向の標準偏差の閾値
	...
	else if (N_guide > 0)
	{
		...
		//誘導者が視界半径内にいるとき、誘導に従う
		if (d_iL_min <= R_vis)
		{
			double rho_L = -(1 / R_vis) * d_iL_min + 1;
		...
		//誘導者の誘導半径内にいるとき、誘導者に接近する
		else if (d_iL_min <= R_ind)
		{
			Vector2d n_iL = unitVector(position, guide[nearestGuideNumber].getPosition());

			desiredDirection = n_iL;
			desiredVelocity = desiredSpeed * desiredDirection;
		}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					static double t_alongWall = 0;
					vector<int> isVisibleWallNumber;				//視認できる壁の番号を格納する配列

					double v_i = sqrt((velocity.x * velocity.x) + (velocity.y * velocity.y));
...
void setInitialPosition_g(const Room room, std::vector<Agent>& guide)
{
	...
	const Vector2d p3 = Vector2d(room_size_x / 2, room_size_y / 4);
	const Vector2d p4 = Vector2d(room_size_x / 2, 0);
	//const Vector2d p6 = Vector2d(room_size_x / 2, -room_size_y / 4);
	const Vector2d p5 = Vector2d(3 * room_size_x / 4, room_size_y / 4);
	const Vector2d p6 = Vector2d(3 * room_size_x / 4, 0);
